...
public class DefaultSettings {
    ...
    public static void loadDefaults (final Map<String, Map<String, String>> sets) {
        ...
        nokiaCsv.put ("inbox", "sms;deliver;\"$(address)\";\"\";\"\";\"$(dateyyyy.MM.dd hh:mm)\";\"\";\"$(body)\"\n");
        nokiaCsv.put ("sent", "sms;submit;\"\";\"$(address)\";\"\";\"$(dateyyyy.MM.dd hh:mm)\";\"\";\"$(body)\"\n");
        sets.put ("Nokia Csv", nokiaCsv);
        final Map<String, String> iPhoneCsv = new HashMap<String, String> ();
        iPhoneCsv.put ("inbox", "\"Received\",\"$(dateM/d/yy)\",\"$(dateh:mm a)\",\"$(address)\",\"[^\"]*\",\"[^\"]*\",\"[^\"]*\",\"$(body)\",\"[^\"]*\"\n");
        iPhoneCsv.put ("sent", "\"Sent\",\"$(dateM/d/yy)\",\"$(dateh:mm a)\",\"$(address)\",\"[^\"]*\",\"[^\"]*\",\"[^\"]*\",\"$(body)\",\"[^\"]*\"\n");
        sets.put ("iPhone Csv", iPhoneCsv);
        final Map<String, String> blackberryCsv = new HashMap<String, String> ();
        blackberryCsv.put ("inbox", "[^,]*,[^,]*,$(dateEEE MMM d HH:mm:ss zzz yyyy),false,$(address),\"$(body)\"\n");
        blackberryCsv.put ("sent", "[^,]*,$(dateEEE MMM d HH:mm:ss zzz yyyy),[^,]*,true,$(address),\"$(body)\"\n");
