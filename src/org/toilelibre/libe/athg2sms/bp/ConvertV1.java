...
public class ConvertV1 extends Thread implements ConvertThread {
    ...
    public void run () {
        ...
        try {
            ...
            br = new BufferedReader (fr);
            final SimpleDateFormat df = new SimpleDateFormat ("yyyy.MM.dd hh:mm", Locale.US);
            fr = new StringReader (this.f);
            br = new BufferedReader (fr);
            ...
            while (line != null) {
                ...
                if ( (message.length > 0) && (message [0].startsWith ("0") || message [0].startsWith ("+"))) {
                    final Map<String, Object> values = new HashMap<String, Object> ();
                    values.put ("address", message [0]);
                    values.put ("date", df.parse (message [3]).getTime ());
                    values.put ("body", message [5].substring (0, message [5].length () - 1));
                    this.activity.insert (new URI (folder), values);
                }
    ...
    public void setConvertListener (final ConvertListener activity) {
