...
class BuiltInFormatsLoader {
    ...
    Map<String, Format> loadDefaults () {
        ...
        formats.put(BuiltInFormatName.MxtSmsglobal.getValue(), new Format(BuiltInFormatName.MxtSmsglobal.getValue(), "[\\s]*\"$(dateyyyy-MM-dd HH:mm:ss)\",[^,]*,$(address),\"$(body)\",$(folder)\n", "\"$(dateyyyy-MM-dd HH:mm:ss)\",0000000000,$(address),\"$(body)\",Delivered[\\s]+", "?", "Delivered"));
        ...
        formats.put(BuiltInFormatName.XmlMessage.getValue(), new Format(BuiltInFormatName.XmlMessage.getValue(), "<Message>\\s*<Recepients(?:[\\s]*\\/>|>\\s*<string>$(sent:address)<\\/string>\\s*<\\/Recepients>)\\s*<Body(?:[\\s]*\\/>|>$(body)<\\/Body>)\\s*<IsIncoming>$(folder)<\\/IsIncoming>\\s*<IsRead>(?:[^<]+)<\\/IsRead>\\s*<Attachments(?:.*?\\/>)\\s*<LocalTimestamp>$(date)<\\/LocalTimestamp>\\s*<Sender(?:[\\s]*\\/>|>$(inbox:address)<\\/Sender>)\\s*<\\/Message>",
